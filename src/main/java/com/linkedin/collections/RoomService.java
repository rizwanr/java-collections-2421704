...
public class RoomService {
	...
	public Collection<Room> getRoomsByCapacity(final int requiredCapacity) {
		...
		Collection<Room> roomsByCapacity = inventory.stream().filter(room -> room.getCapacity()>= requiredCapacity).collect(Collectors.toList());
	...
	public Collection<Room> getRoomByRateAndType(final double rate, final String type){
		...
		Collection<Room> roomsByRateAndType= inventory.stream().filter(room -> room.getRate()<rate).filter(room ->room.getType().equals(type)).collect(Collectors.toList());
		return roomsByRateAndType;
